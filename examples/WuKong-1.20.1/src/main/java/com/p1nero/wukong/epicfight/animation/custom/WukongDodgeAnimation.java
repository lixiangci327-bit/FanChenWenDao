        ...
        this.addEvents(AnimationEvent.TimeStampedEvent.create(delayTime, ((livingEntityPatch, staticAnimation, objects) -> {
            if (livingEntityPatch instanceof ServerPlayerPatch serverPlayerPatch && WukongWeaponCategories.isWeaponValid(livingEntityPatch)) {
                serverPlayerPatch.getOriginal().getCapability(WKCapabilityProvider.WK_PLAYER).ifPresent(wkPlayer -> {
                    ...
                    Boolean isCharging = weaponInnate.getDataManager().getDataValue(WukongSkillDataKeys.IS_CHARGING.get());
                    Boolean PILLAR_isCharging = weaponInnate.getDataManager().getDataValue(WukongSkillDataKeys.IS_CHARGING.get());
                    if (isCharging == null && PILLAR_isCharging == null) {
                        return;
                    }else if(isCharging != null){
                        if (isCharging && wkPlayer.isPerfectDodge() && !isPerfect) {
                        ...
                        weaponInnate.getDataManager().setDataSync(WukongSkillDataKeys.IS_CHARGING.get(), false, serverPlayerPatch.getOriginal());
                    }else if(PILLAR_isCharging != null){
                        if (PILLAR_isCharging && wkPlayer.isPerfectDodge() && !isPerfect) {
                            weaponInnate.getSkill().setConsumptionSynchronize(serverPlayerPatch, 1);
                            weaponInnate.getSkill().setStackSynchronize(serverPlayerPatch, 0);
                        }
                        weaponInnate.getDataManager().setDataSync(WukongSkillDataKeys.IS_CHARGING.get(), false, serverPlayerPatch.getOriginal());
